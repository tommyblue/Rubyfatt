...
describe ConsolidatedTax do
  it "should have taxes correctly sorted by default" do
    ...
    expect(ordered_taxes[0].id).to eq @tax1.id
    expect(ordered_taxes[1].id).to eq @tax2.id
    expect(ordered_taxes[2].id).to eq @tax3.id
    ...
    expect(ordered_taxes[0].id).to eq @tax3.id
    expect(ordered_taxes[1].id).to eq @tax2.id
    expect(ordered_taxes[2].id).to eq @tax1.id
