...
describe Api::V1::InvoicesController do
  context ":index" do
    it "only shows user invoices" do
      user = FactoryGirl.create :user_with_token
      FactoryGirl.create :customer_with_invoices, user: user

      # Create a different user with invoices to check if the controller
      # returns correct invoices
      bad_user = FactoryGirl.create :user
      FactoryGirl.create :customer_with_invoices, user: bad_user
      @request.env['HTTP_AUTHORIZATION'] = user.tokens.last.token
      get :index, format: :json
      json = JSON.parse(response.body)
      expect(json['invoices'].map { |i| i['id'] } ).to eq(user.invoices.order(date: :desc).map(&:id))
    end

    it "can filter invoices by year" do
      user = FactoryGirl.create :user_with_token
      customer = FactoryGirl.create :customer, user: user
      FactoryGirl.create :invoice, customer: customer, date: 2.year.ago
      FactoryGirl.create :invoice, customer: customer, date: 1.year.ago
      FactoryGirl.create :invoice, customer: customer, date: Time.now

      @request.env['HTTP_AUTHORIZATION'] = user.tokens.last.token
      get :index, year: 1.year.ago.year, format: :json
      json = JSON.parse(response.body)
      expect(json['invoices'].map { |i| Date.parse(i['date']).year }.uniq ).to eq([1.year.ago.year])
    end
  end
