class DashboardController < ApplicationController
  def index
    ...
    @this_year = Invoice.select('MONTH(invoices.date) AS month_number,SUM(slips.rate) AS month_income').joins(:slips, :customer).where("customers.user_id = ? AND YEAR(invoices.date) = ? AND invoices.paid = ?", current_user.id, Time.now.year, true).group("MONTH(invoices.date)")
