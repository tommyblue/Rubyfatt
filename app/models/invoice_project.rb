class InvoiceProject < ActiveRecord::Base
  ...
  before_create do
    self.number = self.customer.user.options.where(:name => 'NEXT_INVOICE_PROJECT_NUMBER').first.value.to_i
  end
    
  after_create do
    opt = self.customer.user.options.where(:name => 'NEXT_INVOICE_PROJECT_NUMBER').first
